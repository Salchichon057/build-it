...
export const authService = {
	signUp: async (
		...
		if (!userId) {
			return { data: null, error: "No se pudo obtener el ID del usuario de Supabase Auth." };
		}

		const { error: dbError } = await authRepository.insertUser({
			...userData,
			id: userId,
		} as Omit<User, "created_at">);
		if (dbError) {
